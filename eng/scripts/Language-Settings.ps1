...
function Update-javascript-GeneratedSdks([string]$PackageDirectoriesFile) {
  ...
  foreach ($directory in $moduleFolders) {
    ...
    if (Test-Path $tspLocationPath) {
      ...
      # Initialize management plane modular client flag to false by default
      $isManagementPlaneModularClient = $false
      
      # Read specFolder from generateInput.json and check tspconfig.yaml
      $generateInputPath = Join-Path $RepoRoot "generateInput.json"
      $specFolder = $null
      $tspConfigPath = $null
      $mainTspPath = $null
      
      # Try to read generateInput.json
      if (!(Test-Path $generateInputPath)) {
        Write-Host "No generateInput.json found at $RepoRoot" -ForegroundColor Yellow
        continue;
      }

      try {
        $generateInputJson = Get-Content $generateInputPath -Raw | ConvertFrom-Json
        if (!$generateInputJson.specFolder) {
          Write-Host "No specFolder found in $generateInputPath" -ForegroundColor Yellow
        } else {
          $specFolder = $generateInputJson.specFolder
          Write-Host "Found specFolder in $generateInputPath: $specFolder"
          
          # Check if specFolder exists
          $repoSpecFolderPath = Join-Path $RepoRoot $specFolder
          if (!(Test-Path $repoSpecFolderPath)) {
            Write-Host "specFolder does not exist at $repoSpecFolderPath" -ForegroundColor Yellow
          } else {
            Write-Host "specFolder exists at $repoSpecFolderPath" -ForegroundColor Green
            
            # Look for tspconfig.yaml and main.tsp
            $tspConfigPath = Join-Path $repoSpecFolderPath "tspconfig.yaml"
            $mainTspPath = Join-Path $repoSpecFolderPath "main.tsp"
            
            if (!(Test-Path $tspConfigPath)) {
              Write-Host "No tspconfig.yaml found at $tspConfigPath" -ForegroundColor Yellow
            } else {
              try {
                Write-Host "Found tspconfig.yaml at $tspConfigPath" -ForegroundColor Green
        ...
        Write-Host "Failed to parse $generateInputPath: $_" -ForegroundColor Yellow
      }


      # Only continue processing if it's explicitly a management plane modular client
