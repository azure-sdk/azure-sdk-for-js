...
export class MongoClustersImpl implements MongoClusters {
  ...
  > {
    ...
    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, mongoClusterName, body, options },
      spec: promoteOperationSpec,
    });
    const poller = await createHttpPoller<
      MongoClustersPromoteResponse,
      OperationState<MongoClustersPromoteResponse>
   ...
   * @param options The options parameters.
   */
  async beginPromoteAndWait(
    resourceGroupName: string,
    mongoClusterName: string,
    body: PromoteReplicaRequest,
    options?: MongoClustersPromoteOptionalParams,
  ): Promise<MongoClustersPromoteResponse> {
    const poller = await this.beginPromote(
      resourceGroupName,
      mongoClusterName,
      body,
      options,
...
const promoteOperationSpec: coreClient.OperationSpec = {
  ...
  responses: {
    200: {
      headersMapper: Mappers.MongoClustersPromoteHeaders,
    },
    201: {
      headersMapper: Mappers.MongoClustersPromoteHeaders,
    },
