 ...
 */
async function updatesAPipelineGroupInstance(): Promise<void> {
  const credential = new DefaultAzureCredential();
  const subscriptionId = "00000000-0000-0000-0000-000000000000";
  const client = new MonitorClient(credential, subscriptionId);
  const result = await client.pipelineGroups.update("myResourceGroup", "plGroup1", {
    tags: { tag1: "A", tag2: "B" },
    properties: {
      replicas: 3,
      receivers: [
...
async function main(): Promise<void> {
  await updatesAPipelineGroupInstance();
}
