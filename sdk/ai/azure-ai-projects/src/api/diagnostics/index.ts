...
export function _getAppInsightsSend(
  context: Client,
  resourceName: string,
  options: DiagnosticsGetAppInsightsOptionalParams = { requestOptions: {} },
): StreamableMethod {
  return context
    .path("/components/{resourceName}", resourceName)
...
export async function getAppInsights(
  ...
  const result = await _getAppInsightsSend(context, resourceName, options);
