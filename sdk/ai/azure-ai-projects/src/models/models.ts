  ...
  return {
    ...
    data_sources:
      item["dataSources"] === undefined
        ? item["dataSources"]
        : item["dataSources"].map(vectorStorageDataSourceSerializer),
  };
}

/** The structure to be supplied when the vector store is being created from the Azure asset ID. */
export interface VectorStorageDataSource {
  /** Asset URI. */
  storageUri: string;
  /** The asset type * */
...
export function vectorStorageDataSourceSerializer(
  item: VectorStorageDataSource,
): Record<string, unknown> {
  return {
    uri: item["storageUri"],
    type: item["assetType"],
...
export type VectorStorageDataSourceAssetType = "uri_asset" | "id_asset";
...
export interface FileSearchToolResource {
  ...
  vectorStores?: VectorStoreFromAzure[];
}
...
export function fileSearchToolResourceSerializer(
  ...
  return {
    ...
    vector_stores:
      item["vectorStores"] === undefined
        ? item["vectorStores"]
        : item["vectorStores"].map(vectorStoreFromAzureSerializer),
  };
}

/** Vector store, keeping several vector store configurations. */
export interface VectorStoreFromAzure {
  /** Name */
  storeName: string;
  /** Configurations */
...
export function vectorStoreFromAzureSerializer(
  item: VectorStoreFromAzure,
): Record<string, unknown> {
  return {
    name: item["storeName"],
    configuration: vectorStorageConfigurationSerializer(
...
export interface VectorStorageConfiguration {
  /** Data sources */
  dataSources: VectorStorageDataSource[];
}

export function vectorStorageConfigurationSerializer(
  item: VectorStorageConfiguration,
): Record<string, unknown> {
  return {
    data_sources: item["dataSources"].map(vectorStorageDataSourceSerializer),
  };
}
