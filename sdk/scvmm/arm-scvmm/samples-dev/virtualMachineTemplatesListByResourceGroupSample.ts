 ...
 */
async function virtualMachineTemplatesListByResourceGroupMaximumSet(): Promise<void> {
 ...
 */
async function virtualMachineTemplatesListByResourceGroupMinimumSet(): Promise<void> {
  const credential = new DefaultAzureCredential();
  const subscriptionId = "79332E5A-630B-480F-A266-A941C015AB19";
  const client = new ScVmmClient(credential, subscriptionId);
  const resArray = new Array();
