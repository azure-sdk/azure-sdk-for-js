 ...
 *  this enum contains the known values that the service supports.
 * ### Known values supported by the service
 * **Default value if not selected and means the object's domain name label is not secured against subdomain takeover.**: Unsecure \
 * **Default value if selected and means the object's domain name label can be reused within the same tenant.**: TenantReuse \
 * **Object's domain name label can be reused within the same subscription.**: SubscriptionReuse  \
 * **Object's domain name label can be reused within the same resource group.**: ResourceGroupReuse \
 * **Object's domain name label cannot be reused within the same resource group, subscription, or tenant.**: NoReuse
 */
