...
export default function createClient(
  ...
  { apiVersion = "2023-07-01", ...options }: SchemaRegistryClientOptions = {},
): SchemaRegistryClient {
  const endpointUrl = options.endpoint ?? options.baseUrl ?? `https://${fullyQualifiedNamespace}`;
  ...
  const client = getClient(endpointUrl, credentials, options) as SchemaRegistryClient;
