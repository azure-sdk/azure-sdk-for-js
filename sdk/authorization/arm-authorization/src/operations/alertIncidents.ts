...
export class AlertIncidentsImpl implements AlertIncidents {
  ...
  private async *listForScopePagingPage(
    ...
    while (continuationToken) {
      result = await this._listForScopeNext(scope, alertId, continuationToken, options);
      continuationToken = result.nextLink;
      let page = result.value || [];
      setContinuationToken(page, continuationToken);
      yield page;
  ...
  private async *listForScopePagingAll(
    ...
    for await (const page of this.listForScopePagingPage(scope, alertId, options)) {
      yield* page;
    }
  }
  ...
  private _listForScope(
    ...
    return this.client.sendOperationRequest({ scope, alertId, options }, listForScopeOperationSpec);
  }
...
const listForScopeNextOperationSpec: coreClient.OperationSpec = {
  ...
  urlParameters: [Parameters.$host, Parameters.nextLink, Parameters.scope, Parameters.alertId],
  headerParameters: [Parameters.accept],
  serializer,
};
