...
import {
  chatCompletionStreamOptionsSerializer,
...
export function _getAudioTranscriptionAsPlainTextSend(
  ...
  return context
    ...
    .post({
      ...
      queryParameters: {
        "api-version": options?.apiVersion ?? "2024-10-01-preview",
      },
      body: {
    ...
    .post({
      ...
      body: {
        ...
        stream_options: !body.streamOptions
          ? body.streamOptions
          : chatCompletionStreamOptionsSerializer(body.streamOptions),
        model: body["model"],
        seed: body["seed"],
      },
    });
...
export async function _getCompletionsDeserialize(
  ...
  return {
      ...
      return {
        ...
        contentFilterResults: !p.content_filter_results
          ...
          : {
              ...
              ungroundedMaterial: {
                filtered:
                  p.content_filter_results?.ungrounded_material["filtered"],
                detected:
                  p.content_filter_results?.ungrounded_material["detected"],
                details: p.content_filter_results?.ungrounded_material[
                  "details"
                ].map((p: any) => {
                  return {
                    completionStartOffset: p["completion_start_offset"],
                    completionEndOffset: p["completion_end_offset"],
                  };
                }),
              },
            },
    ...
    usage: {
      ...
      promptTokensDetails: !result.body.usage.prompt_tokens_details
        ? undefined
        : {
            cachedTokens:
              result.body.usage.prompt_tokens_details?.["cached_tokens"],
          },
      completionTokensDetails: !result.body.usage.completion_tokens_details
        ? undefined
        : {
            reasoningTokens:
              result.body.usage.completion_tokens_details?.["reasoning_tokens"],
          },
    },
        ...
        logprobs:
          p.logprobs === null
            ...
            : {
                ...
                refusal:
                  p.logprobs["refusal"] === null
                    ? p.logprobs["refusal"]
                    : p.logprobs["refusal"].map((p: any) => {
                        return {
                          token: p["token"],
                          logprob: p["logprob"],
                          bytes: p["bytes"],
      ...
      result.body["data"] === undefined
        ...
        : result.body["data"].map((p: any) => {
            return {
              ...
              status: p["status"] as BatchStatus,
...
export async function _createBatchDeserialize(
  ...
  return {
    ...
    status: result.body["status"] as BatchStatus,
...
export async function _createUploadDeserialize(
  result: PathUncheckedResponse,
): Promise<Upload> {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
  ...
  return {
    ...
    filename: result.body["filename"],
    bytes: result.body["bytes"],
    purpose: result.body["purpose"] as undefined,
    status: result.body["status"] as undefined,
    expiresAt: new Date(result.body["expires_at"]),
    object: result.body["object"],
    file:
      result.body.file === null
        ? null
        : !result.body.file
          ? undefined
          : {
              object: result.body.file?.["object"],
              id: result.body.file?.["id"],
              bytes: result.body.file?.["bytes"],
              filename: result.body.file?.["filename"],
              createdAt: new Date(result.body.file?.["created_at"]),
              purpose: result.body.file?.["purpose"] as FilePurpose,
              status: result.body.file?.["status"] as FileState,
              statusDetails: result.body.file?.["status_details"],
            },
  };
}
...
export function _addUploadPartSend(
  ...
  return context
    .path("/uploads/{upload_id}/parts", uploadId)
    .post({
      ...operationOptionsToRequestParameters(options),
      contentType: (options.contentType as any) ?? "multipart/form-data",
      body: { data: uint8ArrayToString(requestBody["data"], "base64") },
    });
}
...
export async function _addUploadPartDeserialize(
  ...
  return {
    ...
    object: result.body["object"],
    azureBlockId: result.body["azure_block_id"],
  };
}
...
export async function addUploadPart(
  ...
  const result = await _addUploadPartSend(
    context,
    uploadId,
    requestBody,
    options,
...
export function _completeUploadSend(
  ...
  return context
    .path("/uploads/{upload_id}/complete", uploadId)
    .post({
      ...operationOptionsToRequestParameters(options),
      body: { part_ids: requestBody["partIds"], md5: requestBody["md5"] },
    });
}

export async function _completeUploadDeserialize(
  result: PathUncheckedResponse,
): Promise<Upload> {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
...
export async function completeUpload(
  ...
  const result = await _completeUploadSend(
    context,
    uploadId,
    requestBody,
    options,
...
export function _cancelUploadSend(
  ...
  return context
    .path("/uploads/{upload_id}/cancel", uploadId)
    .post({ ...operationOptionsToRequestParameters(options) });
}

export async function _cancelUploadDeserialize(
  result: PathUncheckedResponse,
): Promise<Upload> {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
