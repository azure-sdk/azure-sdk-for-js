...
export function _listVectorStoreFilesSend(
  ...
  const path = expandUrlTemplate(
    "/vector_stores/{vectorStoreId}/files{?filter,limit,order,after,before}",
    {
      vectorStoreId: vectorStoreId,
      filter: options?.filter,
      limit: options?.limit,
...
export function _listVectorStoresSend(
  ...
  const path = expandUrlTemplate(
    "/vector_stores{?limit,order,after,before}",
    {
      limit: options?.limit,
      order: options?.order,
      after: options?.after,
...
export function _uploadFileSend(
  ...
  return context
    ...
    .post({
      ...
      body: _uploadFileRequestSerializer(file),
    });
}
...
export async function uploadFile(
  ...
  const result = await _uploadFileSend(context, file, options);
...
export function _listRunStepsSend(
  ...
  const path = expandUrlTemplate(
    "/threads/{threadId}/runs/{runId}/steps{?include%5B%5D,limit,order,after,before}",
    {
      threadId: threadId,
      runId: runId,
      "include%5B%5D": !options?.runInclude
        ? options?.runInclude
        : options?.runInclude.map((p: any) => {
            return p;
          }),
      limit: options?.limit,
      order: options?.order,
...
export function _getRunStepSend(
  ...
  const path = expandUrlTemplate(
    "/threads/{threadId}/runs/{runId}/steps/{stepId}{?include%5B%5D}",
    {
      threadId: threadId,
      runId: runId,
      stepId: stepId,
...
export function _listRunsSend(
  ...
  const path = expandUrlTemplate(
    "/threads/{threadId}/runs{?limit,order,after,before}",
    {
      threadId: threadId,
      limit: options?.limit,
      order: options?.order,
...
export function _createRunSend(
  ...
  const path = expandUrlTemplate(
    "/threads/{threadId}/runs{?include%5B%5D}",
    {
      threadId: threadId,
      "include%5B%5D": !options?.runInclude
        ? options?.runInclude
...
export function _listMessagesSend(
  ...
  const path = expandUrlTemplate(
    "/threads/{threadId}/messages{?runId,limit,order,after,before}",
    {
      threadId: threadId,
      runId: options?.runId,
      limit: options?.limit,
...
export function _listAssistantsSend(
  ...
  const path = expandUrlTemplate(
    "/assistants{?limit,order,after,before}",
    {
      limit: options?.limit,
      order: options?.order,
      after: options?.after,
