...
export function _createMessageSend(
  ...
  return context
    ...
    .post({
      ...operationOptionsToRequestParameters(options),
      body: {
        role: threadMessageOptions["role"],
        content: threadMessageOptions["content"],
        attachments:
          threadMessageOptions["attachments"] === undefined ||
          threadMessageOptions["attachments"] === null
            ? threadMessageOptions["attachments"]
            : threadMessageOptions["attachments"].map(
                messageAttachmentSerializer,
              ),
        metadata: !threadMessageOptions.metadata
          ? threadMessageOptions.metadata
          : (serializeRecord(threadMessageOptions.metadata as any) as any),
      },
    });
}
  ...
  return context
    ...
    .post({
      ...
      body: {
        ...
        additional_messages:
          createRunOptions["additionalMessages"] === undefined ||
          createRunOptions["additionalMessages"] === null
            ? createRunOptions["additionalMessages"]
            : createRunOptions["additionalMessages"].map(
                threadMessageSerializer,
              ),
        tools: createRunOptions["tools"],
        stream: createRunOptions["stream"],
        temperature: createRunOptions["temperature"],
        top_p: createRunOptions["topP"],
        max_prompt_tokens: createRunOptions["maxPromptTokens"],
        max_completion_tokens: createRunOptions["maxCompletionTokens"],
        truncation_strategy: !createRunOptions.truncationStrategy
          ? createRunOptions.truncationStrategy
          : truncationObjectSerializer(createRunOptions.truncationStrategy),
        tool_choice: createRunOptions["toolChoice"],
        response_format: createRunOptions["responseFormat"],
        metadata: !createRunOptions.metadata
          ? createRunOptions.metadata
          : (serializeRecord(createRunOptions.metadata as any) as any),
      },
    });
}
