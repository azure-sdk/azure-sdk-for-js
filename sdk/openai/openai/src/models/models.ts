...
export function chatMessageImageContentItemArraySerializer(
  result: Array<ChatMessageImageContentItem>,
): any[] {
  return result.map((item) => {
    return chatMessageImageContentItemSerializer(item);
  });
}
...
export interface ChatMessageContentItem {
  ...
  /** The discriminator possible values: text, refusal, image_url */
  type: string;
}
