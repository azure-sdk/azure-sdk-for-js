...
export interface ToolDefinition {
  /** the discriminator possible values: code_interpreter, file_search, browse, function */
  type: string;
}
...
/** The input definition information for a fie search tool as used to configure an assistant. */
export interface BrowseToolDefinition extends ToolDefinition {
...
export interface Assistant {
  ...
  responseFormat?: string | ("auto" | "none" | AssistantsApiResponseFormat);
  /** A set of up to 16 key/value pairs that can be attached to an object, used for storing additional information about that object in a structured format. Keys may be up to 64 characters in length and values may be up to 512 characters in length. */
  metadata: Record<string, string>;
}
...
/** The response data for a requested list of items. */
export interface OpenAIPageableListOfMessageFile {
  /** The object type, which is always list. */
  object: "list";
  /** The requested list of items. */
  data: MessageFile[];
 ...
 */
export interface RunStepRetrievalToolCall extends RunStepToolCall {
...
export interface RunStepDeltaToolCall {
  ...
  id: string;
  /** the discriminator possible values: function, retrieval, code_interpreter */
  type: string;
}
...
export interface RunStepDeltaRetrievalToolCall extends RunStepDeltaToolCall {
  /** The object type, which is always "retrieval." */
  type: "retrieval";
...
/** "thread.run.created", "thread.run.queued", "thread.run.in_progress", "thread.run.requires_action", "thread.run.completed", "thread.run.failed", "thread.run.cancelling", "thread.run.cancelled", "thread.run.expired" */
export type RunStreamEvent = string;
/** Run step operation related streaming events */
/** "thread.run.step.created", "thread.run.step.in_progress", "thread.run.step.delta", "thread.run.step.completed", "thread.run.step.failed", "thread.run.step.cancelled", "thread.run.step.expired" */
export type RunStepStreamEvent = string;
/** Message operation related streaming events */
/** "thread.message.created", "thread.message.in_progress", "thread.message.delta", "thread.message.completed", "thread.message.incomplete" */
