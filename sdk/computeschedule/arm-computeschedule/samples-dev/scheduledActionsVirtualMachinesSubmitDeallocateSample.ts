 ...
 */
async function scheduledActionsVirtualMachinesSubmitDeallocateMaximumSetGenGeneratedByMinimumSetRule() {
  const credential = new DefaultAzureCredential();
  const subscriptionId = "D8E30CC0-2763-4FCC-84A8-3C5659281032";
  const client = new ComputeScheduleClient(credential, subscriptionId);
  const result = await client.scheduledActions.virtualMachinesSubmitDeallocate(
...
async function main() {
  scheduledActionsVirtualMachinesSubmitDeallocateMaximumSetGenGeneratedByMaximumSetRule();
  scheduledActionsVirtualMachinesSubmitDeallocateMaximumSetGenGeneratedByMinimumSetRule();
}
