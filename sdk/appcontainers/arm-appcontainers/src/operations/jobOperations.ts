...
export class JobOperationsImpl implements JobOperations {
  ...
  async beginRun(
  ...
  ): Promise<PollerLike<PollOperationState<JobRunResponse>, JobRunResponse>> {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ...
    const lro = new LroImpl(
      sendOperation,
      { resourceGroupName, jobName, template, options },
      runOperationSpec
    );
    const poller = new LroEngine(lro, {
      resumeFrom: options?.resumeFrom,
  ...
  private async _executions(
    ...
    PollerLike<PollOperationState<JobExecutionsResponse>, JobExecutionsResponse>
  > {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
...
const executionsNextOperationSpec: coreClient.OperationSpec = {
  ...
  responses: {
    ...
    202: {
      headersMapper: Mappers.JobExecutionsNextHeaders
    },
    default: {
      bodyMapper: Mappers.DefaultErrorResponse
    }
