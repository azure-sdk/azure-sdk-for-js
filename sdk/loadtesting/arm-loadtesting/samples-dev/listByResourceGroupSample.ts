 ...
 * x-ms-original-file: 2023-12-01-preview/LoadTests_ListByResourceGroup.json
 */
async function listLoadTestResourceResourcesByResourceGroup() {
  const credential = new DefaultAzureCredential();
  const subscriptionId = "00000000-0000-0000-0000-000000000000";
  const client = new LoadTestMgmtClient(credential, subscriptionId);
  const resArray = new Array();
  for await (let item of client.listByResourceGroup("dummyrg")) {
    resArray.push(item);
  }
...
async function main() {
  listLoadTestResourceResourcesByResourceGroup();
}
