...
export default function createClient(
  ...
  { apiVersion = "2024-11-30-preview", ...options }: OnlineExperimentationClientOptions = {},
): OnlineExperimentationClient {
  const endpointUrl = options.endpoint ?? options.baseUrl ?? `${endpointParam}`;
  const userAgentInfo = `azsdk-js-azure-analytics-experimentation-rest/1.0.0-beta.1`;
