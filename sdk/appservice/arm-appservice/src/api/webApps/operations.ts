...
export async function _updateMachineKeyDeserialize(result: PathUncheckedResponse): Promise<any> {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    const error = createRestError(result);
    error.details = defaultErrorResponseDeserializer(result.body);
...
export async function _getOneDeployStatusDeserialize(result: PathUncheckedResponse): Promise<any> {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    const error = createRestError(result);
    error.details = defaultErrorResponseDeserializer(result.body);
