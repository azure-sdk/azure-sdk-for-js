...
export class FleetsImpl implements Fleets {
  ...
  public listVirtualMachineScaleSets(
    ...
    const iter = this.listVirtualMachineScaleSetsPagingAll(resourceGroupName, name, options);
    return {
      next() {
        return iter.next();
      },
  ...
  private async *listVirtualMachineScaleSetsPagingPage(
    ...
    let continuationToken = settings?.continuationToken;
    if (!continuationToken) {
      result = await this._listVirtualMachineScaleSets(resourceGroupName, name, options);
      let page = result.value || [];
      continuationToken = result.nextLink;
      setContinuationToken(page, continuationToken);
      yield page;
  ...
  async beginCreateOrUpdate(
    ...
    SimplePollerLike<OperationState<FleetsCreateOrUpdateResponse>, FleetsCreateOrUpdateResponse>
  > {
  ...
  async beginCreateOrUpdateAndWait(
    ...
    const poller = await this.beginCreateOrUpdate(resourceGroupName, fleetName, resource, options);
  ...
  ): Promise<SimplePollerLike<OperationState<FleetsUpdateResponse>, FleetsUpdateResponse>> {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec,
  ...
  async beginUpdateAndWait(
    ...
    const poller = await this.beginUpdate(resourceGroupName, fleetName, properties, options);
    return poller.pollUntilDone();
  }
  ...
  ): Promise<SimplePollerLike<OperationState<FleetsDeleteResponse>, FleetsDeleteResponse>> {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec,
