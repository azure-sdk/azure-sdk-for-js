## API Report File for "@azure/arm-offazurespringboot"

> Do not edit this file. It is a report generated by [API Extractor](https://api-extractor.com/).

```ts

import * as coreAuth from '@azure/core-auth';
import * as coreClient from '@azure/core-client';
import { OperationState } from '@azure/core-lro';
import { PagedAsyncIterableIterator } from '@azure/core-paging';
import { SimplePollerLike } from '@azure/core-lro';

// @public
export type CreatedByType = string;

// @public
export interface ErrorAdditionalInfo {
    readonly info?: Record<string, unknown>;
    readonly type?: string;
}

// @public
export interface ErrorDetail {
    readonly additionalInfo?: ErrorAdditionalInfo[];
    readonly code?: string;
    readonly details?: ErrorDetail[];
    readonly message?: string;
    readonly target?: string;
}

// @public
export interface ErrorModel {
    code?: string;
    id?: number;
    message?: string;
    possibleCauses?: string;
    recommendedAction?: string;
    runAsAccountId?: string;
    severity?: string;
    summaryMessage?: string;
    updatedTimeStamp?: Date;
}

// @public
export interface ErrorResponse {
    error?: ErrorDetail;
}

// @public
export interface ErrorSummaries {
    createOrUpdate(resourceGroupName: string, body: ErrorSummary, options?: ErrorSummariesCreateOrUpdateOptionalParams): Promise<ErrorSummariesCreateOrUpdateResponse>;
    delete(resourceGroupName: string, options?: ErrorSummariesDeleteOptionalParams): Promise<void>;
    get(resourceGroupName: string, options?: ErrorSummariesGetOptionalParams): Promise<ErrorSummariesGetResponse>;
    listBySite(resourceGroupName: string, options?: ErrorSummariesListBySiteOptionalParams): PagedAsyncIterableIterator<ErrorSummary>;
    update(resourceGroupName: string, body: ErrorSummariesResourcePatch, options?: ErrorSummariesUpdateOptionalParams): Promise<ErrorSummariesUpdateResponse>;
}

// @public
export interface ErrorSummariesCreateOrUpdateOptionalParams extends coreClient.OperationOptions {
}

// @public
export type ErrorSummariesCreateOrUpdateResponse = ErrorSummary;

// @public
export interface ErrorSummariesDeleteOptionalParams extends coreClient.OperationOptions {
}

// @public
export interface ErrorSummariesGetOptionalParams extends coreClient.OperationOptions {
}

// @public
export type ErrorSummariesGetResponse = ErrorSummary;

// @public
export interface ErrorSummariesListBySiteNextOptionalParams extends coreClient.OperationOptions {
}

// @public
export type ErrorSummariesListBySiteNextResponse = ErrorSummaryList;

// @public
export interface ErrorSummariesListBySiteOptionalParams extends coreClient.OperationOptions {
}

// @public
export type ErrorSummariesListBySiteResponse = ErrorSummaryList;

// @public
export interface ErrorSummariesProperties {
    discoveryScopeErrorSummaries?: ErrorSummaryModel[];
    errors?: ErrorModel[];
    provisioningState?: ProvisioningState;
}

// @public
export interface ErrorSummariesResourcePatch {
    properties?: ErrorSummariesProperties;
    tags?: {
        [propertyName: string]: string;
    };
}

// @public
export interface ErrorSummariesUpdateOptionalParams extends coreClient.OperationOptions {
}

// @public
export type ErrorSummariesUpdateResponse = ErrorSummary;

// @public
export interface ErrorSummary extends ProxyResource {
    properties?: ErrorSummariesProperties;
    tags?: {
        [propertyName: string]: string;
    };
}

// @public
export interface ErrorSummaryList {
    nextLink?: string;
    value?: ErrorSummary[];
}

// @public
export interface ErrorSummaryModel {
    affectedObjectsCount?: number;
    affectedResourceType?: string;
}

// @public
export function getContinuationToken(page: unknown): string | undefined;

// @public
export enum KnownCreatedByType {
    Application = "Application",
    Key = "Key",
    ManagedIdentity = "ManagedIdentity",
    User = "User"
}

// @public
export enum KnownProvisioningState {
    Accepted = "Accepted",
    Canceled = "Canceled",
    Deleting = "Deleting",
    Failed = "Failed",
    Provisioning = "Provisioning",
    Succeeded = "Succeeded",
    Unknown = "Unknown"
}

// @public (undocumented)
export class OffAzureSpringBoot extends coreClient.ServiceClient {
    // (undocumented)
    $host: string;
    constructor(credentials: coreAuth.TokenCredential, subscriptionId: string, springbootsitesName: string, siteName: string, springbootserversName: string, springbootappsName: string, summaryName: string, errorSummaryName: string, options?: OffAzureSpringBootOptionalParams);
    // (undocumented)
    apiVersion: string;
    // (undocumented)
    errorSummaries: ErrorSummaries;
    // (undocumented)
    errorSummaryName: string;
    // (undocumented)
    operations: Operations;
    // (undocumented)
    siteName: string;
    // (undocumented)
    springbootapps: Springbootapps;
    // (undocumented)
    springbootappsName: string;
    // (undocumented)
    springbootservers: Springbootservers;
    // (undocumented)
    springbootserversName: string;
    // (undocumented)
    springbootsites: Springbootsites;
    // (undocumented)
    springbootsitesName: string;
    // (undocumented)
    subscriptionId: string;
    // (undocumented)
    summaries: Summaries;
    // (undocumented)
    summaryName: string;
}

// @public
export interface OffAzureSpringBootOptionalParams extends coreClient.ServiceClientOptions {
    $host?: string;
    apiVersion?: string;
    endpoint?: string;
}

// @public
export interface Operations {
    list(options?: OperationsListOptionalParams): PagedAsyncIterableIterator<ResourceProviderOperation>;
}

// @public
export interface OperationsListNextOptionalParams extends coreClient.OperationOptions {
}

// @public
export type OperationsListNextResponse = ResourceProviderOperationList;

// @public
export interface OperationsListOptionalParams extends coreClient.OperationOptions {
}

// @public
export type OperationsListResponse = ResourceProviderOperationList;

// @public
export type ProvisioningState = string;

// @public
export interface ProxyResource extends Resource {
}

// @public
export interface Resource {
    readonly id?: string;
    readonly name?: string;
    readonly systemData?: SystemData;
    readonly type?: string;
}

// @public
export interface ResourceProviderOperation {
    display?: ResourceProviderOperationDisplay;
    isDataAction?: string;
    name?: string;
}

// @public
export interface ResourceProviderOperationDisplay {
    description?: string;
    operation?: string;
    provider?: string;
    resource?: string;
}

// @public
export interface ResourceProviderOperationList {
    nextLink?: string;
    value?: ResourceProviderOperation[];
}

// @public
export interface Springbootapps {
    createOrUpdate(resourceGroupName: string, springbootapps: SpringbootappsModel, options?: SpringbootappsCreateOrUpdateOptionalParams): Promise<SpringbootappsCreateOrUpdateResponse>;
    delete(resourceGroupName: string, options?: SpringbootappsDeleteOptionalParams): Promise<SpringbootappsDeleteResponse>;
    get(resourceGroupName: string, options?: SpringbootappsGetOptionalParams): Promise<SpringbootappsGetResponse>;
    listByResourceGroup(resourceGroupName: string, options?: SpringbootappsListByResourceGroupOptionalParams): PagedAsyncIterableIterator<SpringbootappsModel>;
    listBySubscription(options?: SpringbootappsListBySubscriptionOptionalParams): PagedAsyncIterableIterator<SpringbootappsModel>;
    update(resourceGroupName: string, springbootapps: SpringbootappsPatch, options?: SpringbootappsUpdateOptionalParams): Promise<SpringbootappsUpdateResponse>;
}

// @public
export interface SpringbootappsCreateOrUpdateOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootappsCreateOrUpdateResponse = SpringbootappsModel;

// @public
export interface SpringbootappsDeleteOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootappsDeleteResponse = SpringbootappsModel;

// @public
export interface SpringbootappsGetOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootappsGetResponse = SpringbootappsModel;

// @public
export interface SpringbootappsListByResourceGroupNextOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootappsListByResourceGroupNextResponse = SpringbootappsListResult;

// @public
export interface SpringbootappsListByResourceGroupOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootappsListByResourceGroupResponse = SpringbootappsListResult;

// @public
export interface SpringbootappsListBySubscriptionNextOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootappsListBySubscriptionNextResponse = SpringbootappsListResult;

// @public
export interface SpringbootappsListBySubscriptionOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootappsListBySubscriptionResponse = SpringbootappsListResult;

// @public
export interface SpringbootappsListResult {
    nextLink?: string;
    value?: SpringbootappsModel[];
}

// @public
export interface SpringbootappsModel extends ProxyResource {
    properties?: SpringbootappsProperties;
}

// @public
export interface SpringbootappsPatch extends ProxyResource {
    properties?: SpringbootappsProperties;
    tags?: {
        [propertyName: string]: string;
    };
}

// @public
export interface SpringbootappsProperties {
    applicationConfigurations?: SpringbootappsPropertiesApplicationConfigurationsItem[];
    appName?: string;
    appPort?: number;
    appType?: string;
    artifactName?: string;
    bindingPorts?: number[];
    buildJdkVersion?: string;
    certificates?: string[];
    checksum?: string;
    connectionStrings?: string[];
    dependencies?: string[];
    environments?: string[];
    errors?: ErrorModel[];
    instanceCount?: number;
    instances?: SpringbootappsPropertiesInstancesItem[];
    jarFileLocation?: string;
    jvmMemoryInMB?: number;
    jvmOptions?: string[];
    lastModifiedTime?: Date;
    lastUpdatedTime?: Date;
    machineArmIds?: string[];
    miscs?: SpringbootappsPropertiesMiscsItem[];
    provisioningState?: ProvisioningState;
    runtimeJdkVersion?: string;
    servers?: string[];
    siteName?: string;
    springBootVersion?: string;
    staticContentLocations?: string[];
}

// @public (undocumented)
export interface SpringbootappsPropertiesApplicationConfigurationsItem {
    key: string;
    value?: string;
}

// @public (undocumented)
export interface SpringbootappsPropertiesInstancesItem {
    instanceCount?: number;
    jvmMemoryInMB?: number;
    machineArmId: string;
}

// @public (undocumented)
export interface SpringbootappsPropertiesMiscsItem {
    key: string;
    value?: string;
}

// @public
export interface SpringbootappsUpdateOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootappsUpdateResponse = SpringbootappsModel;

// @public
export interface Springbootservers {
    createOrUpdate(resourceGroupName: string, springbootservers: SpringbootserversModel, options?: SpringbootserversCreateOrUpdateOptionalParams): Promise<SpringbootserversCreateOrUpdateResponse>;
    delete(resourceGroupName: string, options?: SpringbootserversDeleteOptionalParams): Promise<SpringbootserversDeleteResponse>;
    get(resourceGroupName: string, options?: SpringbootserversGetOptionalParams): Promise<SpringbootserversGetResponse>;
    listByResourceGroup(resourceGroupName: string, options?: SpringbootserversListByResourceGroupOptionalParams): PagedAsyncIterableIterator<SpringbootserversModel>;
    listBySubscription(options?: SpringbootserversListBySubscriptionOptionalParams): PagedAsyncIterableIterator<SpringbootserversModel>;
    update(resourceGroupName: string, springbootservers: SpringbootserversPatch, options?: SpringbootserversUpdateOptionalParams): Promise<SpringbootserversUpdateResponse>;
}

// @public
export interface SpringbootserversCreateOrUpdateOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootserversCreateOrUpdateResponse = SpringbootserversModel;

// @public
export interface SpringbootserversDeleteOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootserversDeleteResponse = SpringbootserversModel;

// @public
export interface SpringbootserversGetOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootserversGetResponse = SpringbootserversModel;

// @public
export interface SpringbootserversListByResourceGroupNextOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootserversListByResourceGroupNextResponse = SpringbootserversListResult;

// @public
export interface SpringbootserversListByResourceGroupOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootserversListByResourceGroupResponse = SpringbootserversListResult;

// @public
export interface SpringbootserversListBySubscriptionNextOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootserversListBySubscriptionNextResponse = SpringbootserversListResult;

// @public
export interface SpringbootserversListBySubscriptionOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootserversListBySubscriptionResponse = SpringbootserversListResult;

// @public
export interface SpringbootserversListResult {
    nextLink?: string;
    value?: SpringbootserversModel[];
}

// @public
export interface SpringbootserversModel extends ProxyResource {
    properties?: SpringbootserversProperties;
}

// @public
export interface SpringbootserversPatch extends ProxyResource {
    properties?: SpringbootserversProperties;
    tags?: {
        [propertyName: string]: string;
    };
}

// @public
export interface SpringbootserversProperties {
    errors?: ErrorModel[];
    fqdnAndIpAddressList?: string[];
    machineArmId?: string;
    port?: number;
    provisioningState?: ProvisioningState;
    server: string;
    springBootApps?: number;
    totalApps?: number;
}

// @public
export interface SpringbootserversUpdateOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootserversUpdateResponse = SpringbootserversModel;

// @public
export interface Springbootsites {
    beginTriggerRefreshSite(resourceGroupName: string, options?: SpringbootsitesTriggerRefreshSiteOptionalParams): Promise<SimplePollerLike<OperationState<void>, void>>;
    beginTriggerRefreshSiteAndWait(resourceGroupName: string, options?: SpringbootsitesTriggerRefreshSiteOptionalParams): Promise<void>;
    createOrUpdate(resourceGroupName: string, springbootsites: SpringbootsitesModel, options?: SpringbootsitesCreateOrUpdateOptionalParams): Promise<SpringbootsitesCreateOrUpdateResponse>;
    delete(resourceGroupName: string, options?: SpringbootsitesDeleteOptionalParams): Promise<SpringbootsitesDeleteResponse>;
    get(resourceGroupName: string, options?: SpringbootsitesGetOptionalParams): Promise<SpringbootsitesGetResponse>;
    listByResourceGroup(resourceGroupName: string, options?: SpringbootsitesListByResourceGroupOptionalParams): PagedAsyncIterableIterator<SpringbootsitesModel>;
    listBySubscription(options?: SpringbootsitesListBySubscriptionOptionalParams): PagedAsyncIterableIterator<SpringbootsitesModel>;
    update(resourceGroupName: string, springbootsites: SpringbootsitesPatch, options?: SpringbootsitesUpdateOptionalParams): Promise<SpringbootsitesUpdateResponse>;
}

// @public
export interface SpringbootsitesCreateOrUpdateOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootsitesCreateOrUpdateResponse = SpringbootsitesModel;

// @public
export interface SpringbootsitesDeleteOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootsitesDeleteResponse = SpringbootsitesModel;

// @public
export interface SpringbootsitesGetOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootsitesGetResponse = SpringbootsitesModel;

// @public
export interface SpringbootsitesListByResourceGroupNextOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootsitesListByResourceGroupNextResponse = SpringbootsitesListResult;

// @public
export interface SpringbootsitesListByResourceGroupOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootsitesListByResourceGroupResponse = SpringbootsitesListResult;

// @public
export interface SpringbootsitesListBySubscriptionNextOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootsitesListBySubscriptionNextResponse = SpringbootsitesListResult;

// @public
export interface SpringbootsitesListBySubscriptionOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootsitesListBySubscriptionResponse = SpringbootsitesListResult;

// @public
export interface SpringbootsitesListResult {
    nextLink?: string;
    value?: SpringbootsitesModel[];
}

// @public
export interface SpringbootsitesModel extends TrackedResource {
    extendedLocation?: SpringbootsitesModelExtendedLocation;
    properties?: SpringbootsitesProperties;
}

// @public
export interface SpringbootsitesModelExtendedLocation {
    name?: string;
    type?: string;
}

// @public
export interface SpringbootsitesPatch extends TrackedResource {
    properties?: SpringbootsitesProperties;
    tags?: {
        [propertyName: string]: string;
    };
}

// @public
export interface SpringbootsitesProperties {
    masterSiteId?: string;
    migrateProjectId?: string;
    provisioningState?: ProvisioningState;
}

// @public
export interface SpringbootsitesTriggerRefreshSiteOptionalParams extends coreClient.OperationOptions {
    resumeFrom?: string;
    updateIntervalInMs?: number;
}

// @public
export interface SpringbootsitesUpdateOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SpringbootsitesUpdateResponse = SpringbootsitesModel;

// @public
export interface Summaries {
    createOrUpdate(resourceGroupName: string, body: Summary, options?: SummariesCreateOrUpdateOptionalParams): Promise<SummariesCreateOrUpdateResponse>;
    delete(resourceGroupName: string, options?: SummariesDeleteOptionalParams): Promise<void>;
    get(resourceGroupName: string, options?: SummariesGetOptionalParams): Promise<SummariesGetResponse>;
    listBySite(resourceGroupName: string, options?: SummariesListBySiteOptionalParams): PagedAsyncIterableIterator<Summary>;
    update(resourceGroupName: string, body: SummariesResourcePatch, options?: SummariesUpdateOptionalParams): Promise<SummariesUpdateResponse>;
}

// @public
export interface SummariesCreateOrUpdateOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SummariesCreateOrUpdateResponse = Summary;

// @public
export interface SummariesDeleteOptionalParams extends coreClient.OperationOptions {
}

// @public
export interface SummariesGetOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SummariesGetResponse = Summary;

// @public
export interface SummariesListBySiteNextOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SummariesListBySiteNextResponse = SummaryList;

// @public
export interface SummariesListBySiteOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SummariesListBySiteResponse = SummaryList;

// @public
export interface SummariesProperties {
    discoveredApps?: number;
    discoveredServers?: number;
    errors?: ErrorModel[];
    provisioningState?: ProvisioningState;
}

// @public
export interface SummariesResourcePatch {
    properties?: SummariesProperties;
    tags?: {
        [propertyName: string]: string;
    };
}

// @public
export interface SummariesUpdateOptionalParams extends coreClient.OperationOptions {
}

// @public
export type SummariesUpdateResponse = Summary;

// @public
export interface Summary extends ProxyResource {
    properties?: SummariesProperties;
    tags?: {
        [propertyName: string]: string;
    };
}

// @public
export interface SummaryList {
    nextLink?: string;
    value?: Summary[];
}

// @public
export interface SystemData {
    createdAt?: Date;
    createdBy?: string;
    createdByType?: CreatedByType;
    lastModifiedAt?: Date;
    lastModifiedBy?: string;
    lastModifiedByType?: CreatedByType;
}

// @public
export interface TrackedResource extends Resource {
    location: string;
    tags?: {
        [propertyName: string]: string;
    };
}

// (No @packageDocumentation comment for this package)

```
