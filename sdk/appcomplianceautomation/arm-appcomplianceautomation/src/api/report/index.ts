...
> {
  return context
    ...
    .put({
      ...
      body: { properties: reportPropertiesSerializer(parameters.properties) },
    });
}
...
export async function _reportCreateOrUpdateDeserialize(
  ...
  const res = result as unknown as ReportCreateOrUpdateLogicalResponse;
  return {
    id: res.body["id"],
    name: res.body["name"],
    type: res.body["type"],
    ...
    properties: {
      triggerTime: new Date(res.body.properties["triggerTime"]),
      timeZone: res.body.properties["timeZone"],
      resources: res.body.properties["resources"].map((p) => {
        return {
          resourceId: p["resourceId"],
          resourceType: p["resourceType"],
          resourceKind: p["resourceKind"],
      ...
      errors: res.body.properties["errors"],
      tenantId: res.body.properties["tenantId"],
      offerGuid: res.body.properties["offerGuid"],
      nextTriggerTime:
        res.body.properties["nextTriggerTime"] !== undefined
          ? new Date(res.body.properties["nextTriggerTime"])
          : undefined,
      lastTriggerTime:
        res.body.properties["lastTriggerTime"] !== undefined
          ? new Date(res.body.properties["lastTriggerTime"])
          : undefined,
      subscriptions: res.body.properties["subscriptions"],
      complianceStatus: !res.body.properties.complianceStatus
        ? undefined
        : {
            m365: !res.body.properties.complianceStatus?.m365
              ? undefined
              : {
                  passedCount:
                    res.body.properties.complianceStatus?.m365?.["passedCount"],
                  failedCount:
                    res.body.properties.complianceStatus?.m365?.["failedCount"],
                  manualCount:
                    res.body.properties.complianceStatus?.m365?.["manualCount"],
                  notApplicableCount:
                    res.body.properties.complianceStatus?.m365?.[
                      "notApplicableCount"
                    ],
                  pendingCount:
                    res.body.properties.complianceStatus?.m365?.[
      ...
      storageInfo: !res.body.properties.storageInfo
        ? undefined
        : {
            subscriptionId: res.body.properties.storageInfo?.["subscriptionId"],
            resourceGroup: res.body.properties.storageInfo?.["resourceGroup"],
            accountName: res.body.properties.storageInfo?.["accountName"],
            location: res.body.properties.storageInfo?.["location"],
          },
      certRecords:
        res.body.properties["certRecords"] === undefined
          ? res.body.properties["certRecords"]
          : res.body.properties["certRecords"].map((p) => {
              return {
                offerGuid: p["offerGuid"],
                certificationStatus: p["certificationStatus"],
                ingestionStatus: p["ingestionStatus"],
...
export async function _reportUpdateDeserialize(
  ...
  const res = result as unknown as ReportUpdateLogicalResponse;
  return {
    id: res.body["id"],
    name: res.body["name"],
    type: res.body["type"],
...
export async function _reportListDeserialize(
  result: ReportList200Response | ReportListDefaultResponse,
...
export async function _reportSyncCertRecordDeserialize(
  ...
  const res = result as unknown as ReportSyncCertRecordLogicalResponse;
  return {
    certRecord: !res.body.certRecord
      ? undefined
      : {
          offerGuid: res.body.certRecord?.["offerGuid"],
          certificationStatus: res.body.certRecord?.["certificationStatus"],
          ingestionStatus: res.body.certRecord?.["ingestionStatus"],
          controls:
            res.body.certRecord?.["controls"] === undefined
              ? res.body.certRecord?.["controls"]
              : res.body.certRecord?.["controls"].map((p) => {
                  return {
                    controlId: p["controlId"],
                    controlStatus: p["controlStatus"],
                  };
...
export async function _reportFixDeserialize(
  ...
  const res = result as unknown as ReportFixLogicalResponse;
  return {
...
export async function _reportVerifyDeserialize(
  ...
  const res = result as unknown as ReportVerifyLogicalResponse;
  return {
    result: res.body["result"],
    reason: res.body["reason"],
  };
