...
export interface GenerateLandingZoneRequest {
  ...
  /** Optional parent for management group hierarchy, used as intermediate root management group parent, if specified. If empty, default will deploy beneath tenant toot management group. */
 ...
 *  this enum contains the known values that the service supports.
 * ### Known values supported by the service
 * **Authoring**: This is the default status when a create configuration request is made. In this state, the Landing Zone configuration can be updated. \
 * **ReadyForUse**: This status is set when the configuration changes are complete and the configuration is ready for code generation \
 * **Disabled**: This status is set when the landing zone configuration is no longer in use.
 */
 ...
 *  this enum contains the known values that the service supports.
 * ### Known values supported by the service
 * **None**: Value when a firewall resource is not created. \
 * **Standard**: Value when the firewall creation is based on the standard SKU. \
 * **Premium**: Value when the firewall creation is based on the premium SKU.
 */
 ...
 *  this enum contains the known values that the service supports.
 * ### Known values supported by the service
 * **azureFirewalls**: Value when customer wants to provide custom naming convention for Azure Firewall. \
 * **workspaces**: Value when customer wants to provide custom naming convention for Log Analytics workspace. \
 * **automationAccounts**: Value when customer wants to provide custom naming convention for automation account. \
 * **dashboards**: Value when customer wants to provide custom naming convention for dashboard. \
 * **userAssignedIdentities**: Value when customer wants to provide custom naming convention for managed identity. \
 * **bastionHosts**: Value when customer wants to provide custom naming convention for Bastion. \
 * **ddosProtectionPlans**: Value when customer wants to provide custom naming convention for DDoS protection plan. \
 * **networkSecurityGroups**: Value when customer wants to provide custom naming convention for Bastion NSG. \
 * **virtualNetworks**: Value when customer wants to provide custom naming convention for virtual network. \
 * **routeTables**: Value when customer wants to provide custom naming convention for route table.
 */
 ...
 *  this enum contains the known values that the service supports.
 * ### Known values supported by the service
 * **Bicep**: Value when generating Landing Zone code in Bicep. \
 * **ARM**: Value when generating landing zone code in Arm.
 */
