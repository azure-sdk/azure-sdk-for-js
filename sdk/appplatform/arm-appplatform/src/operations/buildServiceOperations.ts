...
import { BuildServiceOperations } from "../operationsInterfaces/index.js";
...
export class BuildServiceOperationsImpl implements BuildServiceOperations {
  ...
  private async *listBuildServicesPagingPage(
    ...
    if (!continuationToken) {
      result = await this._listBuildServices(resourceGroupName, serviceName, options);
  ...
  private async *listBuildsPagingPage(
    ...
    let continuationToken = settings?.continuationToken;
    if (!continuationToken) {
      result = await this._listBuilds(resourceGroupName, serviceName, buildServiceName, options);
      let page = result.value || [];
      continuationToken = result.nextLink;
      setContinuationToken(page, continuationToken);
      yield page;
