...
import { ProgrammableConnectivityClient } from "./clientDefinitions.js";
...
export default function createClient(
  ...
  {
    apiVersion = "2024-02-09-preview",
    ...options
  }: ProgrammableConnectivityClientOptions = {},
): ProgrammableConnectivityClient {
  const endpointUrl = options.endpoint ?? options.baseUrl ?? `${endpointParam}`;
  const userAgentInfo = `azsdk-js-programmableconnectivity-rest/1.0.0-beta.1`;
  ...
  const client = getClient(
    ...
    credentials,
    options,
  ) as ProgrammableConnectivityClient;
