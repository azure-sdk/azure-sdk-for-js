...
export interface MongoClustersPromote202Headers {
  /** The Location header contains the URL where the status of the long running operation can be checked. */
  location?: string;
  /** The Retry-After header can indicate how long the client should wait before polling the operation status. */
  "retry-after"?: number;
...
export interface MongoClustersPromote202Response extends HttpResponse {
  status: "202";
  headers: RawHttpHeaders & MongoClustersPromote202Headers;
}

export interface MongoClustersPromoteDefaultResponse extends HttpResponse {
  status: string;
  body: ErrorResponseOutput;
}

/** The final response for long-running promote operation */
export interface MongoClustersPromoteLogicalResponse extends HttpResponse {
  status: "200";
}
...
/** Azure operation completed successfully. */
export interface ReplicasListByParent200Response extends HttpResponse {
  status: "200";
  body: ReplicaListResultOutput;
}

export interface ReplicasListByParentDefaultResponse extends HttpResponse {
  status: string;
  body: ErrorResponseOutput;
}
