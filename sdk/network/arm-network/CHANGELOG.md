...
### Breaking Changes
  ...
  - Operation AdminRules.createOrUpdate has a new signature
  - Interface AdminRuleCollection no longer has parameter systemData
  - Interface BaseAdminRule no longer has parameter systemData
  - Interface SecurityAdminConfiguration no longer has parameter systemData
  - Parameter access of interface AdminRule is now required
  - Parameter direction of interface AdminRule is now required
  - Parameter priority of interface AdminRule is now required
  - Parameter protocol of interface AdminRule is now required
  - Type of parameter value of interface AdminRuleListResult is changed from BaseAdminRuleUnion[] to AdminRule[]
