...
export class DiscountsImpl implements Discounts {
  ...
  public listResourceGroupList(
    ...
    return {
      ...
      byPage: (settings?: PageSettings) => {
        ...
        return this.resourceGroupListPagingPage(resourceGroupName, options, settings);
      },
    };
  }
  ...
  private async *resourceGroupListPagingPage(
    ...
    while (continuationToken) {
      result = await this._resourceGroupListNext(resourceGroupName, continuationToken, options);
      continuationToken = result.nextLink;
      let page = result.value || [];
      setContinuationToken(page, continuationToken);
      yield page;
  ...
  private async *resourceGroupListPagingAll(
    ...
    for await (const page of this.resourceGroupListPagingPage(resourceGroupName, options)) {
      yield* page;
    }
  }
  ...
  private _scopeList(
    ...
    return this.client.sendOperationRequest({ scope, options }, scopeListOperationSpec);
  }
  ...
  async beginCreateAndWait(
    ...
    const poller = await this.beginCreate(resourceGroupName, discountName, body, options);
  ...
  async beginDeleteAndWait(
    ...
    const poller = await this.beginDelete(resourceGroupName, discountName, options);
    return poller.pollUntilDone();
  }
  ...
  async beginCancelAndWait(
    ...
    const poller = await this.beginCancel(resourceGroupName, discountName, options);
    return poller.pollUntilDone();
  }
