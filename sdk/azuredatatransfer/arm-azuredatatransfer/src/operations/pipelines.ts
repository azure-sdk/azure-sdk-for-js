...
export class PipelinesImpl implements Pipelines {
  ...
  > {
    ...
    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, pipelineName, resource, options },
  ...
  async beginUpdate(
    ...
    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, pipelineName, properties, options },
  ...
  async beginUpdateAndWait(
    ...
    const poller = await this.beginUpdate(resourceGroupName, pipelineName, properties, options);
    return poller.pollUntilDone();
  }
  ...
  async beginDeleteAndWait(
    ...
    const poller = await this.beginDelete(resourceGroupName, pipelineName, options);
    return poller.pollUntilDone();
  }
  ...
  > {
    ...
    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, pipelineName, body, options },
  ...
  > {
    ...
    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, pipelineName, body, options },
      spec: executeActionOperationSpec,
    });
    const poller = await createHttpPoller<
      PipelinesExecuteActionResponse,
      OperationState<PipelinesExecuteActionResponse>
    >(lro, {
      restoreFrom: options?.resumeFrom,
      intervalInMs: options?.updateIntervalInMs,
      resourceLocationConfig: "location",
   ...
   * @param options The options parameters.
   */
  async beginExecuteActionAndWait(
    resourceGroupName: string,
    pipelineName: string,
    body: Action,
    options?: PipelinesExecuteActionOptionalParams,
  ): Promise<PipelinesExecuteActionResponse> {
    const poller = await this.beginExecuteAction(resourceGroupName, pipelineName, body, options);
    return poller.pollUntilDone();
  }
  ...
  listSchemasList(
    ...
    return this.client.sendOperationRequest(
      { resourceGroupName, pipelineName, body, options },
      listSchemasListOperationSpec,
    );
  }
  ...
  async beginRejectConnectionAndWait(
    ...
    const poller = await this.beginRejectConnection(resourceGroupName, pipelineName, body, options);
    return poller.pollUntilDone();
  }
...
const executeActionOperationSpec: coreClient.OperationSpec = {
...
const listSchemasListOperationSpec: coreClient.OperationSpec = {
