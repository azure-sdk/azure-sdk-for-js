...
In order to release it, we need to add some tests for it to make sure we are delivering high quality packages. After generation you will see starter test scaffolding in your package's `test/` folder, which you can update with cases against your own service.
...
1. **Write the test**
    ...
    - RLC example: [Maps Route tests](https://github.com/Azure/azure-sdk-for-js/tree/main/sdk/maps/maps-route-rest/test)
    - DPG example: [Key Vault Keys tests](https://github.com/Azure/azure-sdk-for-js/tree/main/sdk/keyvault/keyvault-keys/test)
    - MPG example: [AVS tests](https://github.com/Azure/azure-sdk-for-js/tree/main/sdk/avs/arm-avs/test)
...
2. **Run the test**
    ...
    On Linux or macOS, you could use `export` to set env variable:

    ```shell
    pnpm turbo build --filter=${PACKAGE_NAME}... --token 1
    export TEST_MODE=record && pnpm test # this records tests and updates assets referenced by assets.json after you push recordings.
    ```
    ...
    ```shell
    pnpm turbo build --filter=${PACKAGE_NAME}... --token 1
    SET TEST_MODE=record&& pnpm test # this records tests and updates assets referenced by assets.json after you push recordings.
    ```
    ...
    ```shell
    pnpm turbo build --filter=${PACKAGE_NAME}... --token 1
    export TEST_MODE=playback && pnpm test # this runs the tests against previously pushed recordings.
    ```
    On Windows, you can use:

    ```shell
    pnpm turbo build --filter=${PACKAGE_NAME}... --token 1
    SET TEST_MODE=playback&& pnpm test # this runs the tests against previously pushed recordings.
    ```

# How to write samples

We highly encourage you to write some valid samples for your customer to get started with your service libraries. You may author TypeScript samples under the `samples-dev` folder.

To learn more, you could refer to the below samples:
- DPG sample: [the samples of Key Vault Keys](https://github.com/Azure/azure-sdk-for-js/tree/main/sdk/keyvault/keyvault-keys/samples-dev)
- RLC sample: [the samples of Maps Route](https://github.com/Azure/azure-sdk-for-js/tree/main/sdk/maps/maps-route-rest/samples-dev)
...
Now, you can generate both JavaScript and TypeScript runnable samples with the following commands.
...
CHANGELOG can help customers know the change of new version quickly, so you need to update it according to the change of this new version. The top changelog entry should use `(Unreleased)` until the package is actually released.
